...
% normalized voltage thresholds and save timings to folders in plotV_allbcls.m
% instead of here. It's easier to perform a visual check on the values using the other script. 
% Apr 2020: Modified again (created _feedbacktests.m version) to produce longer 
% reference trajectories for certain BCLs. In the previous version there
% was no distinction between fixedpointfolder_read and
% fixedpointfolder_write. In addition, I don't think re-running nsoli is
% necessary over larger numbers of cycles when we already have a shifted fp IC
% available from past runs, so I commented that part out. 
...
fixedpointfolder_read = ['lrddata_fromfixedpointsearch_highres' shiftstring '/']; %obtain info here
...
select_bcl_shortlist = 1000; % Reduce list for feedback tests
...
bclindices = 1:L;
select_bcl_shortlist_indices = bclindices(selected_bcls_for_fps == select_bcl_shortlist);
...
fixedpointfolder_write = ['lrddata_fromfixedpointsearch_highres' shiftstring '_ncyc' num2str(ncyc) '/']; %store trajectories from longer simulations here, for use with feedback tests
...
load([fixedpointfolder_read 'threshold_times.mat'],'i_depol','i_repol');

%In case program crashed partway through, check for previously saved fixed
%points:
if exist([fixedpointfolder_read 'compiled_fp.mat'],'file')
    eval(['load ' fixedpointfolder_read 'compiled_fp.mat fp_found']);
end

if ~exist(fixedpointfolder_write,'dir')
    mkdir(fixedpointfolder_write)
end
...
for i = select_bcl_shortlist_indices %startindex:L %1:L
...
%     end
    ...
    if 0%fperrnorm > tolfactor*nsolitol
%        tstartfp=tic;
        % Find fixed point:
...
%        toc(tstartfp);
        ...
        if ierr == 0
            ...
            eval(['save ' fixedpointfolder_write 'compiled_fp.mat *']);
        else
...
eval(['movefile ' modelinputfolder '* ' fixedpointfolder_write]);

load([fixedpointfolder_write 'lrddata_1cell_b' num2str(select_bcl_shortlist(1))],'Y') 
figure
plot((1:size(Y,2))*data.dt,Y(1,:))
